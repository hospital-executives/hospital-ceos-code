...
code_path <- paste0(params$code_path, "/")
data_path <- paste0(params$data_path, "/")
...
first_five <- head(df, 5)
write.csv(first_five, targets$outliers)
write.csv(first_five, targets$confirmed_r)
write.csv(first_five, targets$remaining_r)
```

```{r multiple_names, eval = FALSE}
...
write.csv(outliers, targets$confirmed_r)
write.csv(outliers, targets$remaining_r)
```

```{r multiple_ids, eval = FALSE}
all_objects <- ls()
...
final_confirmed <- final_confirmed %>%
  select(-c(add_list, entity_name_list, entityid_list))
write_feather(final_confirmed, paste0(data_path, "derived/r_confirmed.feather"))

final_remaining <- final_remaining %>%
  select(-c(add_list, entity_name_list, entityid_list))
write_feather(final_remaining, paste0(data_path, "derived/r_remaining.feather"))
