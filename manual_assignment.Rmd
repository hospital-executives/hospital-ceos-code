...
date: "2024-07-10"
params:
  input_path: "/Users/loaner/BFI Dropbox/Katherine Papen/hospital_ceos/"
---
...
# user_directory <- "/Users/loaner/BFI Dropbox/Katherine Papen/hospital_ceos/_data/"
# user_directory <- "/Users/gard/BFI Dropbox/Michelle Gard/hospital_ceos/_data"
user_directory <- paste0(input_path, "_data/")

# load main df
df <- read_feather(paste0(user_directory, 
...
outliers <- read.csv(paste0(user_directory, "derived/temp_one_id_m_names.csv"))

# set up one id m names
normalized_levenshtein_cutoff <- 0.3 # default is 0.3
# source("/Users/loaner/BFI Dropbox/Katherine Papen/hospital_ceos/_code/one_id_m_names_final.R") # update
#source("/Users/gard/BFI Dropbox/Michelle Gard/hospital_ceos/_code/one_id_m_names_final.R") # update
source(paste0(input_path, "_code/one_id_m_names_final.R"))
# set up one name m ids
# source("/Users/loaner/BFI Dropbox/Katherine Papen/hospital_ceos/_code/one_name_m_ids_final.R") # update
#source("/Users/gard/BFI Dropbox/Michelle Gard/hospital_ceos/_code/one_name_m_ids_final.R") # update
source(paste0(input_path, "_code/one_name_m_ids_final.R"))
```

```{r multiple_names, echo=FALSE}
...
## run time ~ 3.5 mins
one_id_m_names_output1 <- manual_one_id_m_names(df) 
...
write.csv(outliers, paste0(user_directory, "derived/temp_one_id_m_names.csv"), row.names = FALSE)
```


```{r multiple_ids, echo=FALSE}
all_objects <- ls()
objects_to_keep <- c("df", "outliers", "user_directory", "input_path")
objects_to_remove <- setdiff(all_objects, objects_to_keep)
rm(list = objects_to_remove)
# source("/Users/loaner/BFI Dropbox/Katherine Papen/hospital_ceos/_code/one_name_m_ids_final.R")
#source("/Users/gard/BFI Dropbox/Michelle Gard/hospital_ceos/_code/one_name_m_ids_final.R")
source(paste0(input_path, "_code/one_name_m_ids_final.R"))
...
one_name_m_ids_output1 <- matching_one_name_m_ids(df, outliers) 
...
all_objects <- ls()
objects_to_keep <- c("df", "outliers", "user_directory", "one_m_confirmed1", 
                     "one_id", "one_state_confirmed", "one_state_per_year_confirmed",
                     "mult_state_per_year_confirmed", "one_state_remaining",
                            "one_state_per_year_remaining",
                            "mult_state_per_year_remaining", "mult_id")
objects_to_remove <- setdiff(all_objects, objects_to_keep)
rm(list = objects_to_remove)
...
final_confirmed <- bind_rows(one_m_confirmed1, one_ids_clean, one_state_confirmed,
                      one_state_per_year_confirmed, mult_state_per_year_confirmed)
...
final_confirmed <- final_confirmed %>%
  select(-c(add_list, entity_name_list, entityid_list))
#write.csv(final_confirmed, paste0(user_directory, "derived/himss_entity_contacts_0517_confirmed"), row.names = FALSE)
write_feather(final_confirmed, paste0(user_directory, "derived/confirmed_test.feather"))

final_remaining <- final_remaining %>%
  select(-c(add_list, entity_name_list, entityid_list))
write_feather(final_remaining, paste0(user_directory, "derived/remaining_test.feather"))
