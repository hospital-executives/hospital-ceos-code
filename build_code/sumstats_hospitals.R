...
himss_path <- args[1]
haentity_path <- args[2]
dta_path <- args[3]
output_path <- args[4]

himss_data <- read_feather(himss_path)
haentity <- read_feather(haentity_path)
dta <- read_stata(dta_path)
summary_file <- paste0(output_path, "/summary.tex")
...
for (nm in names(dfs)) {
  ...
  ggsave(
    filename = file.path(output_path, paste0("/merge_status_by_year_", nm, ".png")),
...
for (nm in names(dfs)) {
  ...
  ggsave(
    filename = file.path(output_path, paste0("/hospital_counts_by_year_", nm, ".png")),
    plot = p, width = 8, height = 5, dpi = 300
  )
...
cat(
  ...
  sprintf("%.2f\\%%", n_distinct(ever_match$entity_uniqueid) / num_hospitals * 100),
  ").\n",
  file = summary_file, append = TRUE
)
...
ggsave(paste0(output_path, "/entity_match.png"), plot = himss_entity_plot, width = 6, height = 4, dpi = 300)
...
ggsave(paste0(output_path, "/entity_match_by_year.png"), 
       plot = entity_by_year, width = 6, height = 4, dpi = 300)

cat(
  "\nWe have a match for",
  sprintf(
    "%.2f\\%%",
    100 * sum((fuzzy_graph_by_year %>% filter(match_type == "fuzzy_match"))$count) /
...
cat("\nThere are", nrow(dont_merge), 
    "cases that are assigned an entity AHA but don't merge to the AHA.",
    file = summary_file, append = TRUE)
...
cat(
  ...
  sprintf("%.2f\\%%", 100 * length(overlap) / length(aha_ahanumber)),
  ") out of", length(aha_ahanumber), "ahanumbers in the AHA data.\n",
  file = summary_file, append = TRUE
)
...
cat(
  ...
  "\nthere is ever a match on", length(overlap), "ahanumbers, (",
  sprintf("%.2f\\%%", length(overlap) / length(aha_ahanumber) * 100),
  ") out of", length(aha_ahanumber), "ahanumbers in the AHA data.\n",
  file = summary_file, append = TRUE
)
...
kable(combined_table, format = "latex", booktabs = TRUE,
  ...
  cat(file = paste0(output_path, "/type_breakdown_comparison.tex"))
