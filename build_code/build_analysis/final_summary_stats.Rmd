...
ha_tagged <-himss_hospitals %>% filter(!is.na(entity_aha) & !is.na(mname)) %>%
...
hosp_year_pairs <- himss_hospitals %>%
  filter(haentitytypeid == 1) %>% distinct(entity_uniqueid, year)

cat("There are ", nrow(hosp_year_pairs), "in HIMSS")
...
cat("We have a match for", 
    sum((fuzzy_graph_by_year %>% filter(match_type == "fuzzy_match"))$count)/
      nrow(himss_hospitals %>% filter(haentitytypeid == 1) %>% distinct(entity_uniqueid, year)))

## remaining cases
dont_merge <- himss_hospitals %>% filter(haentitytypeid == 1) %>%
  filter(!is.na(entity_aha) & is.na(mname)) %>% distinct(entity_uniqueid, year)

campus_no_entity <- himss_hospitals %>% filter(haentitytypeid == 1) %>%
  filter(is.na(entity_aha) & !is.na(campus_aha))

never_assigned <- himss_hospitals %>% filter(haentitytypeid == 1) %>% 
...
full_xwalk <- read_feather(paste0(derived_data,'/himss_aha_hospitals_final.feather'))
...
cat("There are", nrow(aha_ahanumber), "hospital-year pairs in the AHA.")
cat("There is a match on ", nrow(overlap)/nrow(aha_ahanumber), 
    "hospital-year pairs,  or", nrow(overlap))
```

```{r drop government}
check_vets <- us_only %>% filter((sysid != "9295" & sysid!= "9395")| is.na(sysid)) 
all_vets <- us_only %>% filter(sysid == "9295"|sysid== "9395")
