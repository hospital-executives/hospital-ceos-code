...
hospitals <- read_feather(paste0(data_path,'derived/himss_aha_hospitals_final.feather'))
...
## subset to only (aha, year) observations that are in HIMSS
...
cat(nrow(all_aha_ceos))

## first pass - match HIMSS CEOs in same year
...
total_combos <- cleaned_joined %>% 
  distinct(full_aha, aha_year, aha_aha)
...
get_count <- remaining2 %>% distinct(full_aha,aha_aha, aha_year)
...
valid_himss <- check_himss %>% filter(!is.na(entity_aha)) %>%
  rename(
    first_himss = firstname,
    last_himss = lastname,
...
still_missing <- valid_himss %>% anti_join(
  title_mismatch %>% distinct(ahanumber, full_aha, year)
)

get_count <- still_missing %>% distinct(full_aha,ahanumber, year)

## get fuzzy year match
library(dplyr)
library(stringdist)

## confirmed
new_confirmed <- rbind(accounted_for %>% rename(ahanumber = aha_aha,
                                                year = aha_year), 
...
# 1.  Keep only HIMSS rows within ± 2 years of each AHA row ---------------
aha_df  <- cleaned_aha %>%
  anti_join(new_confirmed %>% select(-match_type)) %>%
  rename(aha_year  = year)
...
within_2yr <- aha_df %>%                       # every AHA row …
  left_join(himss_df, by = "ahanumber") %>%    #   … with candidate HIMSS rows
  filter(abs(himss_year - aha_year) <= 2)       # keep those in the window

# 2.  Jaro-Winkler distances for each pair ---------------------------------
...
# 3.  Helper that returns NA instead of Inf when everything is missing -----
min_dist <- function(x) if (all(is.na(x))) NA_real_ else min(x, na.rm = TRUE)

# 4.  One summary row per (full_aha, ahanumber, year) ----------------------
...
all_matches <- rbind(new_confirmed, matches) %>%
  mutate(
...
matches <-  cleaned_remaining %>% rename(year = aha_year) %>%
  left_join(aha_sysid) %>%
...
check <- remaining %>% filter(campus_aha != ahanumber)

check_aha <- test_missing %>% filter(ahanumber == 6110002)
## verify using aha system id
aha_sysid <- cleaned_aha %>% distinct(ahanumber, year, sysid)

check_with_sys <- remaining %>% rename(year = aha_year) %>%
  left_join(aha_sysid) %>%
  rename(aha_aha = ahanumber,
         aha_sysid = sysid,
         ahanumber = campus_aha,
...
# check all matches
get_count <- all_matches %>% distinct(full_aha,ahanumber, year)
...
## check merge on firstname and lastname
