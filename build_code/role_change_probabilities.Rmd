---
title: "role_change_probabilities"
output: html_document
date: "2024-08-10"
---

## Role Change Probabilities
Find the likelihood of role to role transitions (e.g. frequency someone goes from COO to CEO, or from Facility Head to Chief Medical Officer)
...
# First, replace the user_directory value with your own file path to DropBox
# This will be added to the beginning of the file paths for the dropbox files this script will be accessing
source("/Users/gard/BFI Dropbox/Michelle Gard/hospital_ceos/_code/config.R")
```

# Load data
Use Katherine's confirmed dataset to calculate role to role transitions within contacts

```{r load confirmed data}
#use Katherine's confirmed dataset
file_name <- "/temp_confirmed_0813.csv"
file_path <- paste0(derived_data,file_name)
confirmed_data <- read_csv(file_path)

#load full dataset for reference
file_name <- "/himss_entities_contacts_0517_v1.feather"
file_path <- paste0(derived_data,file_name)
entity_contacts <- read_feather(file_path)
```

```{r create job hierarchy}
...
```{r probability of transitions across years}
#probabilities
...
```{r probability of jobs held simultaneously}
...
ggplot(job_pair_probabilities, aes(x = probability)) +
  geom_histogram(binwidth = 0.01, fill = "blue", color = "black") +
  labs(title = "Histogram of Job Pair Transition Probabilities",
       x = "Probability",
       y = "Frequency") +
  theme_minimal()
