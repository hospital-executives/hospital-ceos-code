...
summary_file <- paste0(output_dir, "/non_ceo_summary.tex")
...
himss_processed <- final %>% filter(!is.na(entity_aha)) %>%
  ...
  inner_join(all_aha_no_ceos, by = c("ahanumber", "year")) %>%
...
valid_himss <- himss_processed %>%
  rename(first_himss = firstname, last_himss = lastname) %>%
  mutate(full_jw = stringdist(full_name, full_aha, method = "jw", p = 0.1),
...
# get nonmatched ceos
remaining_aha_madmin <- all_aha_no_ceos %>% 
  anti_join(aha_matches %>% select(-match_type)) %>%
  rename(full_name = full_aha)

himss_csuite <- final %>% 
  filter(str_detect(title_standardized, "CFO|CEO|COO|CIO|Chief Medical")) %>%
  group_by(contact_uniqueid, full_name) %>%
  slice(1) %>%
  ungroup()

# join ceos on full name
joined <- remaining_aha_madmin %>% 
  stringdist_left_join(himss_csuite,
                       by = "full_name",
                       method = "jw",
                       max_dist = .2,
...
stopifnot(nrow(total_combos) == nrow(remaining_aha_madmin))

# get name similarities
fuzzy_names_madmin <- cleaned_joined %>%
...
year_mismatches <- fuzzy_names_madmin %>%
  group_by(full_aha, aha_aha, aha_year) %>%
...
#### clear environment ####
keep_names <- c(
  "all_aha_ceos", "all_aha_no_ceos", "cleaned_aha", "all_matches", "matched_himss",
  "final",
  "dict1", "dict2", "dict3",
...
remaining <- all_aha_no_ceos %>% left_join(cleaned_aha) %>%
...
# get unaccounted for madmin
unmatched <- all_aha_no_ceos %>% anti_join(confirmed_matches) %>% 
...
# save himss matches 
write_feather(matched_himss, paste0(auxiliary_data, "/matched_aha_no_ceos_himss.feather"))
...
print(
  ...
  file = paste0(output_dir, "/aha_non_ceos.tex"),
  include.rownames = FALSE
)
write.csv(df_counts, "temp/aha_non_ceos.csv", row.names = FALSE)
...
ggsave(paste0(output_dir, "/aha_non_ceos_to_himss.png"), plot = p, width = 6, height = 4, dpi = 300)
