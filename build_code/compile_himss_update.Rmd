...
```{r}
# write haentity to feather
write_feather(haentity, paste0(auxiliary_data, "/haentity.feather"))
system2("python", args = c("mcr_aha_xwalk.py", data_file_path))
...
haentity_not_hosp <- haentity %>%
  #filter(is.na(medicarenumber))%>% 
  filter(haentitytypeid != "1")
         
haentity_updated <- haentity %>% filter(haentitytypeid == "1")
...
) # this is not handled by pyscript

# Handle the rows with non-NA medicarenumber
haentity_not_na <- haentity %>%
  filter(!is.na(medicarenumber)) %>%
  left_join(aha_xwalk, by = c("medicarenumber", "year")) %>% # this is what you need to change
