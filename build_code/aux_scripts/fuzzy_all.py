...
subgraph = confirmed_graph.subgraph(cleaned_ids_from_confirmed).copy()
cleaned_df = input_df[~input_df['contact_uniqueid'].isin(cleaned_ids_from_confirmed)]

# now need to test ids that werent there orginally
py_ids = set(py_confirmed['contact_uniqueid'])
new_cleaned = cleaned_ids_from_confirmed - py_ids
