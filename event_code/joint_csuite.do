...
gen any_csuite_turnover = (turnover_cfo == 1 | turnover_cmo == 1 | turnover_coo == 1 | ///
    ...
    if !missing(turnover_cfo) & !missing(turnover_cmo) & !missing(turnover_coo) & ///
       !missing(turnover_cco) & !missing(turnover_cno)

gen turnover_ceo_x_any = (turnover_ceo == 1 & any_csuite_turnover == 1) ///
    if !missing(turnover_ceo) & !missing(any_csuite_turnover)
gen ceo_not_any = (turnover_ceo == 1 & any_csuite_turnover == 0) ///
    if !missing(turnover_ceo) & !missing(any_csuite_turnover)
...
twoway ///
    ...
    subtitle("`spec1_name' | Joint Avg: `joint_avg_fmt' (SE: `joint_se_fmt') | Non-joint Avg: `nonjoint_avg_fmt' (SE: `nonjoint_se_fmt')", size(small)) ///
    legend(order(1 "Non-joint (CEO only)" 2 "Joint (CEO x Any C-Suite)") rows(1) position(6)) ///
    graphregion(color(white))
